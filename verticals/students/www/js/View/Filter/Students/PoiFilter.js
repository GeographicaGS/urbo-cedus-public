    ...
    this.asyncModel.fetch({
      success: function(ranges) {
        ...
        ranges.each(function(range) {
          ...
          var valueRange = range.get('value');

          // Real number of colleges mustn't show in detail view
          if (status.get('id') === '29' && 
            this.model.get('the_geom') &&
            this.model.get('the_geom').ST_Intersects) {
            valueRange = '1';
          }
          $('.statusesTypes li[data-id="' + status.get('id') + '"] .total').html(valueRange)
        }.bind(this));
      }.bind(this)
    });
    var _this = this;
